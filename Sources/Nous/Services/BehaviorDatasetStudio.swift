...
enum BehaviorDatasetStudio {
    ...
    static func persist(
        ...
        let cases = try cases.map(normalizedCase)
        ...
        for behaviorCase in cases {
            guard let caseId = normalizedCaseId(behaviorCase.id) else {
            ...
            if behaviorCase.origin == .incident {
                currentIncidentIds.insert(caseId)
            }
        }
        ...
        return BehaviorDatasetWriteSummary(
    ...
    private static func normalizedCase(_ behaviorCase: BehaviorDatasetCase) throws -> BehaviorDatasetCase {
        guard let id = normalizedCaseId(behaviorCase.id) else {
            throw BehaviorDatasetStudioError.invalidCaseId(behaviorCase.id)
        }
        return BehaviorDatasetCase(
            id: id,
            ...
            sourceCaseId: behaviorCase.sourceCaseId.flatMap(normalizedCaseId),
            user: behaviorCase.user,
            assistant: behaviorCase.assistant,
            expectedBehavior: behaviorCase.expectedBehavior,
    ...
    private static func normalizedCaseId(_ id: String) -> String? {
    ...
    private static func existingCaseIds(in urls: [URL]) throws -> Set<String> {
        ...
        for url in urls where FileManager.default.fileExists(atPath: url.path) {
            ...
            for line in contents.split(separator: "\n") {
                ...
                guard let id = normalizedCaseId(id) else {
